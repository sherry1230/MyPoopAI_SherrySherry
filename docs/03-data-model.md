...
- 게스트 → 회원 전환은 Firebase 익명 인증 → `linkWithCredential` 계정 연결로 데이터 승계.
  회원가입 유인: "기기 바꿔도 기록과 단어집이 그대로" = 동기화가 곧 가입 보상

> ⚠ 충돌 메모: 기획서 9/4판 §6은 "게스트 = 클라이언트 저장 / 회원 = Firestore 동기화(확정)"로
> 표기. 그러나 현행 구현(2026-07-29 확정)은 **게스트 포함 전부 서버(Firestore) 저장**
> (익명 uid, LocalStorage 기록 정책 폐기)이다. 어느 쪽이 최종인지 운영자 확인 필요 —
> 확인 전 코드 롤백 금지.
