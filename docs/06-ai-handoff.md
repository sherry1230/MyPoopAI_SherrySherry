...
>
> 제품 기준 원본: 구글닥 기획서 **v0.10** (2026-07-27) — 저장소 내 사본: `docs/gdocs/` (읽기 전용, `/sync-docs`로 동기화)
...
가족 계정(성인 프로필 추가)은 **보류 확정** (v0.10 — 간병 시나리오만 추후 별도 검토).
...
- **AI 메모리 (뇌 3분할 + v0.10 기억의 소유권)**: ① 프로필 뇌 — users.profile 매 호출 통째 주입
  ② 배변 건강 뇌 — records + **tool use(기록_검색·기간_통계)로 실제 DB 조회** (환각 방지: 날짜를
  지어내지 않는다) + 패턴 요약 배치(users.patternSummary 상시 주입) ③ 엔티티 뇌 — entities 컬렉션
  (인물·사물 노드, 이름 매칭 회수). **①②는 캐릭터 공유(인수인계는 건강 데이터만), ③은 캐릭터별
  분리 보관**(`entities.character [pupu|pipi]`, 회수 시 현재 캐릭터 것만 주입 — 전환 시 삭제가
  아니라 보관). 엔티티는 대화 로그와 동일 최고 보안 등급
