...
- **게스트 = 익명 인증(uid 발급) + 기록은 클라이언트 저장 / 회원 = Firestore 동기화**
  (2026-09-04 운영자 확정. ⚠ 현행 코드는 게스트도 Firestore 저장 — 클라이언트 저장 전환 백로그)
- 가입 전환은 **linkWithCredential로 uid 유지** + 클라이언트 기록 업로드 승계.
  가입 즉시 잠겨 있던 히스토리가 열리고 기기 간 동기화 시작 (쌓인 기록 + 동기화가 보상)
- **전 계정(게스트 포함) 회원탈퇴 필수**: 원본 이미지·개인정보·대화 로그 즉시 삭제, analytics는 비식별 보존

## BM (기획서 9/4판 기준)

- 사진 분석·저장: **1일 1회 무료** / **텍스트 기록·분석: 무제한** (저비용 간단 분류 —
  2026-09-04 운영자 확정). 추가 사진 분석 = 리워드 광고 시청.
...
> ✅ 2026-09-04 확정: 사진 1일 1회 + 텍스트 무제한 (구 v0.10 쿼터표 폐기).
...
- **회원** 기록을 LocalStorage에 저장 (회원은 Firestore 동기화가 원칙 —
  게스트 기록의 클라이언트 저장은 9/4 확정 정책이므로 허용)
...
- 📘 **기획서 9/4판(닝겐 기획서 PDF) 반영**: 5-Brain 구조, 친밀도=노드수 확정, Haema 노드
  ...
  충돌 3건은 2026-09-04 운영자 확정으로 해소: ① BM = 사진 1일 1회 + 텍스트 무제한
  ② 게스트 = 클라이언트 저장(코드 전환 백로그) ③ 초기 1회는 채팅 온보딩, 재방문(로그인+기록)은 카메라 자동 오픈
